...
# Panel 2: Companion with tabs (Forum / Notes / AI Connection — Forum active)
px = panel_x[1]
...
tabs = [("Forum", True), ("Notes", False), ("AI Connection", False)]
tab_w = PW // 3
...
# Group context card (matches reading-context style)
ctx_h = 50
d.rounded_rectangle((left_pad, cy, right_pad, cy + ctx_h), 8, fill=(238, 243, 255), outline=(216, 227, 255), width=1)
d.text((left_pad + 12, cy + 8), "GROUP", fill=(107, 135, 194), font=f(9, bold=True))
d.text((left_pad + 12, cy + 20), "First Baptist · Tuesday Study", fill=BLUE, font=f(13, bold=True))
d.text((left_pad + 12, cy + 36), "12 members · Reading: John 3", fill=(107, 135, 194), font=f(10))
cy += ctx_h + 14


# Initials avatar (colored circle with letters)
def init_avatar(cx, cy_, r, initials, color):
    d.ellipse((cx - r, cy_ - r, cx + r, cy_ + r), fill=color)
    ft = f(int(r * 0.95), bold=True)
    bb = d.textbbox((0, 0), initials, font=ft)
    d.text((cx - (bb[2] - bb[0]) // 2, cy_ - (bb[3] - bb[1]) // 2 - 2), initials, fill=WHITE, font=ft)
...
def post_card(initials, color, name, role, when, body, ref, replies, likes, pinned=False):
    global cy
    ft_body = f(11)
    inner_left = left_pad + 12 + 36 + 10  # avatar + gap
    body_max = right_pad - 12 - inner_left
    body_lines = wrap_text(body, ft_body, body_max)
    h = 12 + (16 if pinned else 0) + 18 + 4 + len(body_lines) * 16 + 8 + 22 + 12
    border = (216, 227, 255) if pinned else LIGHT
    bg = (247, 250, 255) if pinned else WHITE
    d.rounded_rectangle((left_pad, cy, right_pad, cy + h), 10, fill=bg, outline=border, width=1)
    inner_y = cy + 12
    if pinned:
        d.text((left_pad + 12, inner_y), "PINNED", fill=BLUE, font=f(9, bold=True))
        inner_y += 14
    # avatar
    init_avatar(left_pad + 12 + 18, inner_y + 14, 18, initials, color)
    # name + role + time
    d.text((inner_left, inner_y), name, fill=TEXT, font=f(12, bold=True))
    name_w = d.textbbox((0, 0), name, font=f(12, bold=True))[2]
    if role:
        d.text((inner_left + name_w + 6, inner_y + 1), role, fill=BLUE, font=f(10, bold=True))
    # time on right
    when_bb = d.textbbox((0, 0), when, font=f(10))
    d.text((right_pad - 12 - (when_bb[2] - when_bb[0]), inner_y + 2), when, fill=(150, 150, 153), font=f(10))
    # body
    yy = inner_y + 22
    ...
    # reference pill
    if ref:
        bb = d.textbbox((0, 0), ref, font=f(10, bold=True))
        rw = bb[2] - bb[0] + 16
        d.rounded_rectangle((inner_left, yy, inner_left + rw, yy + 20), 10, fill=(238, 243, 255))
        d.text((inner_left + 8, yy + 4), ref, fill=BLUE, font=f(10, bold=True))
    # reply / like counts on right
    meta = f"reply  {replies}     like  {likes}"
    bb = d.textbbox((0, 0), meta, font=f(10))
    d.text((right_pad - 12 - (bb[2] - bb[0]), yy + 4), meta, fill=(150, 150, 153), font=f(10))
    cy += h + 8


# Posts
post_card(
    "MA", (29, 108, 243), "Pastor Mike A.", "Leader", "5h",
    "This week's discussion: John 3 — what does it mean to be 'born again' in your own walk? Looking forward to hearing from each of you.",
    "John 3:3", 12, 8, pinned=True,
)
post_card(
    "SK", (243, 92, 132), "Sarah K.", "", "2h",
    "Verse 5 has been on my mind: 'born of water and spirit.' I always read this as baptism but recently heard a teaching that connects 'water' to physical birth. Has anyone else explored this?",
    "John 3:5", 4, 3,
)
post_card(
    "DL", (62, 178, 122), "David L.", "", "1h",
    "John 3:16 is the verse I memorized as a kid, but reading it again — 'whoever believes in him should not perish' — I'm struck by how present-tense 'believes' is.",
    "John 3:16", 2, 7,
)
post_card(
    "ER", (192, 132, 252), "Emma R.", "", "15m",
    "↳ Reply to David: 'believing' as ongoing trust really changed how I read this. There's a Greek nuance to the present tense that supports that reading.",
    "John 3:16", 0, 2,
)
...
d.text((left_pad + 12, inp_y + 8), "Share your thoughts on John 3…", fill=(180, 180, 180), font=f(12))
...
img.save("/home/user/churches/preview-forum.png")
print("saved preview-forum.png", img.size)
